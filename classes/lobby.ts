...
export class Lobby {
    ...
    updateEmbed(embed: EmbedBuilder, rmembeds: boolean = true): void {
        if(rmembeds) {
            this._msg!.edit({ embeds: [embed], components: [] }).catch(error => console.log('Lobby deleted - cannot edit'));
        } else {
            this._msg!.edit({ embeds: [embed] }).catch(error => console.log('Lobby deleted - cannot edit'));
        }
    }
