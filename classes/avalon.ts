...
export class Avalon extends Lobby
{
    ...
    setup(thread: AnyThreadChannel): void {
        ...
        thread.send({ embeds: [embed], components: [row] }).catch(error => console.log('Permissions were revoked midgame'));
    ...
    nextMission() {
        ...
        this._thread?.send({ embeds: [embed], components: [row] }).catch(error => console.log('Permissions were revoked midgame'));
    }
    ...
    finishGame(reswin: boolean): void {
        ...
        this._thread?.send({ embeds: [embed] }).catch(error => console.log('Permissions were revoked midgame'));
